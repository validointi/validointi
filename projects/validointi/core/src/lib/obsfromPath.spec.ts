...
describe('Objects from path', () => {
  ...
  it('should throw on an invalid path', () => {
    ...
    expect(() => objFromPath('a. .b', 1)).toThrowError(
      `[@validointi] Invalid path: "a. .b", should not contain whitespace between the dots or brackets`
    );
    expect(() => objFromPath('a..b', 1)).toThrowError(
      `[@validointi] Invalid path: "a..b", it contains two dots in a row`
    );
    expect(() => objFromPath('[ ]', 1)).toThrowError(
      `[@validointi] Invalid path: "[ ]", should not contain whitespace between the dots or brackets`
    );
    expect(() => objFromPath('a.[ \n].b', 1)).toThrowError(
      `[@validointi] Invalid path: "a.[ \n].b", should not contain whitespace between the dots or brackets`
    );
  });
});
