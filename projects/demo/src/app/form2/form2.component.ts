...
import { SampleData, SampleDataForm2Service, type SwapiPeople } from './sample-data-form2.service';
@Component({
  selector: 'app-form2',
  standalone: true,
...
export class Form2Component {
  ...
  starwars$ = this.#httpClient
    .get<SwapiPeople[]>(`https://swapi.info/api/people`)
    .pipe(map((results) => results.map((person, index) => ({ id: index, name: person.name }))));
